...
{
	#region variables extension
		 
	private double anguloCodo;
	private double anguloHombro;
	double diferencia;
	
	private int statusPostura =0;
	private float timePosture=0;
	private double lastAngle=0;
	private int repeticionHombro=0;
	
	public string textoValidacion="";

	GameObject AngulosExtensiones;
	...
	void Start()
	{
		CalibrationText = GameObject.Find("InfoData");
		AngulosExtensiones=GameObject.Find("InfoData");
		int hr = 0;
		...
		if(CalibrationText != null)
		{
			CalibrationText.GetComponent<GUIText>().text = "Waiting for User!!";
		}
...
//			interactionManager.StopInteraction();
    ...
    void OnGUI()
    {
		//GUI.Label (new Rect (10, 10, 100, 200), "Hello Worlds!");
...
//						}
					...
					#region calculoExtensionPiernaDerecha
					if(AngulosExtensiones != null){
				
			
						//double difZJoints = Math.Abs((100/player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.ShoulderRight].z)-player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.WristRight].z);
						//double limZJointsShoulder = player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.ShoulderRight].z/10;
						//calcula la validacion de la postura en hombro lateral
											
anguloHombro=Math.Round(CalcularAngulo( 
					player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.WristRight],
					player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.ShoulderRight],
					player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.HipCenter]),0);										

anguloCodo=Math.Round(CalcularAngulo( 
					player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.ShoulderRight],
					player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.ElbowRight],
					player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.WristRight]),0);							
						
diferencia=Math.Round(Math.Abs(player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.ShoulderRight].z-
					player1JointsPos[(int)KinectWrapper.NuiSkeletonPositionIndex.ElbowRight].z),2);					
					
				
						
				if(diferencia>=0.15){
					AngulosExtensiones.GetComponent<GUIText>().text ="Postura Incorrecta";
				}else{
							
					switch(statusPostura){
							case 0:
							if(anguloHombro-10>10&&anguloHombro+10<50){
									statusPostura=1;
							}
								break;
							case 1:
							if(anguloHombro-10>110&&anguloHombro+10<150){
								statusPostura=2;
							}
								break;
							case 2:
					...
					lastAngle=anguloHombro;
					AngulosExtensiones.GetComponent<GUIText>().text ="GESTURE INFO\n===================\nCurrent Time:"+currentNuiTime+"\nDelta Time:"+deltaNuiTime+"\n" +
						"Shoulder Degree:"+anguloHombro+"\nElbow Degree:"+anguloCodo+"\nGesture Status:"+statusPostura+"\nCompletes: "+repeticionHombro;
				}
	...
	#region Metodos Tec
		 ...
		 private double CalcularAngulo(Vector3 joint1, Vector3 joint2,Vector3 joint3)
	        {
	            Vector3 head =joint1;
	            Vector3 centerShoulder =joint2;
	            Vector3 spine=joint3;
	
	            double angle = 0;
	            double shrhX = head.x - centerShoulder.x;
	            double shrhY = head.y - centerShoulder.y;
	            double shrhZ = head.z - centerShoulder.z;
				double hsl = NormalizarVector(shrhX, shrhY, shrhZ);
	            double unrhX = spine.x - centerShoulder.x;
	            double unrhY = spine.y - centerShoulder.y;
	            double unrhZ = spine.z - centerShoulder.z;
	            double hul = NormalizarVector(unrhX, unrhY, unrhZ);
				...
				if (x != Double.NaN)
	            {
	                if (-1 <= x && x <= 1)
	                {
	                    double angleRad = System.Math.Acos(x);
	                    angle = angleRad * (180.0 / System.Math.PI);
	                }
	                else
	                {
	                    angle = 0;
		...
		public double NormalizarVector(double x, double y, double z)
	        {
